...
/* 
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        double randomFirst = StdRandom.uniform();
        // first day has 50% chance being wet

        // if the first day is wet, it fills the first index of the forecast array with the first probability   
        if (randomFirst < .5) {
        ...
        for (int i = 0; i < numberOfDays-1; i++) {
            ...
            if (forecast[i] == DRY && random <= drywetProbability) {
                forecast[i+1] = WET;
            }
            else if (forecast[i] == DRY && random > drywetProbability) {
                forecast[i+1] = DRY;
            }

            if (forecast[i] == WET && random <= wetwetProbability) {
                forecast[i+1] = WET;
            }
            else if (forecast[i] == WET && random > wetwetProbability) {
                forecast[i+1] = DRY;
            }
            }
            return forecast;
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for(int i = 0; i < forecast.length; i++) { /*
            ...
            for(int j = i; i < forecast.length; i++)
            else{
                for(int j = i; j < i + 7; j++) {
