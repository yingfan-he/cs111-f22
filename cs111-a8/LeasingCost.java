public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int name = description.indexOf("name:")+5;  // targets the variable in the txtfile, gives you the index of the first character in "name", the +5 makes it go past the colon
        String nameStart = description.substring(name); // grabs everything after name:
        int nameEnd = nameStart.indexOf(";");
        String carName = nameStart.substring(0, nameEnd);

        int lease = description.indexOf("length:")+7;
        String leaseStart = description.substring(lease);
        int leaseEnd = leaseStart.indexOf(";");
        String leaseLength = leaseStart.substring(0, leaseEnd);

        int type = description.indexOf("type:")+5;
        String typeStart = description.substring(type);
        int typeEnd = typeStart.indexOf(";");
        String fuelType = typeStart.substring(0, typeEnd);

        int usage = description.indexOf("mile/unit:")+10;
        String usageStart = description.substring(usage);
        int usageEnd = usageStart.indexOf(";");
        String usageMiles = usageStart.substring(0, usageEnd);

        int charger = description.indexOf("charger:")+8;
        String chargerStart = description.substring(charger);
        int chargerEnd = chargerStart.indexOf(";");
        String chargerCost = chargerStart.substring(0, chargerEnd);

        int due = description.indexOf("due:")+4;
        String dueStart = description.substring(due);
        int dueEnd = dueStart.indexOf(";");
        String dueAtSigning = dueStart.substring(0, dueEnd);

        int monthly = description.indexOf("monthly:")+8;
        String monthlyStart = description.substring(monthly);
        int monthlyEnd = monthlyStart.indexOf(";");
        String monthlyCost = monthlyStart.substring(0, monthlyEnd);

        int allowance = description.indexOf("allowance:")+10;
        String allowanceStart = description.substring(allowance);
        int allowanceEnd = allowanceStart.indexOf(";");
        String mileageAllowance = allowanceStart.substring(0,allowanceEnd);




        Fuel MyFuelType = new Fuel(Double.parseDouble(usageMiles)); // gas

        if (fuelType.equals("electric")) { // electric
            MyFuelType = new Fuel(Double.parseDouble(usageMiles), Double.parseDouble(chargerCost));
        }

        Lease MyLeaseType = new Lease(Double.parseDouble(dueAtSigning), Integer.parseInt(leaseLength), Double.parseDouble(monthlyCost),Integer.parseInt(mileageAllowance));

        Vehicle myVehicle = new Vehicle(carName, MyFuelType, MyLeaseType);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
       ...
       int numberofMonths = (int)numberofMonth;
       ...
       // calculating and assigning for gas cars
       if(vehicles[i].getFuel().getType()==1) { // if car is gas
        double CO2Emission = computeCO2(numberofMonths, usage, mileageAllowance, Fuel.CO2EMITTED_GASCOMBUSTION);
        vehicles[i].setCO2Emission(CO2Emission);

        double FuelCost = computeFuelCost(numberofMonths, usage, mileageAllowance, gasPrice);
        ...
        double CO2Emission = computeCO2(numberofMonths, usage, mileageAllowance, Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION);
        vehicles[i].setCO2Emission(CO2Emission);

        double FuelCost = computeFuelCost(numberofMonths, usage, mileageAllowance, electricityPrice);
        vehicles[i].setFuelCost(FuelCost);

        double leaseCost = computeLeaseCost(dueAtSigning, numberofMonths, monthlyCost);
        double totalCost = FuelCost + leaseCost + vehicles[i].getFuel().getCharger();
        vehicles[i].setTotalCost(totalCost);
       }
